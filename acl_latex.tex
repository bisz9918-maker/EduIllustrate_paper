...
\begin{abstract}
Generating high-quality illustrated explanations for K-12 STEM problems requires both geometrically accurate diagrams and coherent step-by-step reasoning---a combination that existing LLM-based educational content systems have not addressed. We present \textbf{EduIllustrate}, a benchmark for evaluating LLMs' ability to generate interleaved text-diagram explanations for K-12 STEM problems. The benchmark comprises \textbf{230 problems} spanning three grade levels and five subjects, a standardized four-stage generation protocol with sequential anchoring to enforce cross-diagram visual consistency, and an \textbf{8-dimension evaluation rubric} grounded in multimedia learning theory covering both text quality and visual quality. Evaluation of ten LLMs shows Gemini 3.0 Pro Preview achieves the highest overall score (87.8\%), while Kimi-K2.5 offers the best cost-efficiency (80.8\% at \$0.12/problem). Workflow ablation confirms sequential anchoring improves Visual Consistency by 13\% over a fully parallel baseline at 94\% lower cost. Human evaluation with 20 expert raters validates LLM-as-judge reliability for objective dimensions ($\rho=0.89$ for logical coherence, $\rho=0.83$ for diagram-problem alignment), while revealing limitations on subjective visual assessment.
\end{abstract}
...
We address these gaps with \textbf{EduIllustrate} (Figure~\ref{fig:framework}), a benchmark for systematically evaluating LLMs' ability to generate
interleaved text-diagram explanations for K-12 STEM problems.
The benchmark includes a standardized generation protocol in which
...
\begin{itemize}
  \item EduIllustrateBench, comprising 230 curated problems across five subjects and three grade levels, with an 8-dimension evaluation rubric addressing the gap in multi-subject, multi-grade multimodal educational content evaluation.
  \item A standardized generation protocol with sequential anchoring to ensure cross-diagram visual consistency, serving as both the generation method and an ablation baseline.
  \item Comparative analysis of ten LLMs spanning proprietary and open-weight models, ablation studies,
    and human evaluation validating LLM-as-judge reliability.
\end{itemize}
