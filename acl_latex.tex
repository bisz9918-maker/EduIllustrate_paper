...
\begin{comment}
%% =====================================================================
%% INTRO 修改大纲（根据老师反馈更新，2026-03）
%% =====================================================================
%%
%% 老师反馈六点：
%% (1) 突出multimodal reasoning + interleaved（不只是"生成"）
%% (2) 强调K-12 education gap（当前工作都是大学级别）
%% (3) benchmark覆盖面很重要，要突出
%% (4) evaluation rubric gap要说清楚
%% (5) "精确"diagram与文字交织是独特贡献
%% (6) Manim不是亮点，不要放在标题/亮点位置
%%
%% =====================================================================
%% 【P1】K-12图文解析的重要性 + 生产困境
%% =====================================================================
%%
%% 【句1】图文结合的认知价值（三篇经典理论，不写具体数字）：
%%   \citep{sweller1988cognitive}  — Cognitive Load Theory
%%   \citep{paivio1971imagery}     — Dual Coding Theory
%%   \citep{mayer2001multimedia}   — Multimedia Learning
%%
%% 【句2】生产困境（三层证据）：
%%   层1 — 时间成本: \citep{philipp2013teachers}, \citep{thompson2024teachers}
%%   层2 — 能力壁垒: \citep{turkozudincer2025literacy}
%%   层3 — 系统性缺口: \citep{yao2026instructional}
%%
%% → 草稿句（禁止具体数字）:
%%   However, producing such materials at scale remains a formidable challenge:
%%   lesson planning and material preparation are among the most time-consuming
%%   aspects of teachers' work \citep{philipp2013teachers,thompson2024teachers},
%%   and many educators lack the specialized skills required to create high-quality
%%   illustrated explanations \citep{turkozudincer2025literacy}, resulting in a
%%   systematic shortage of such materials for K-12 students \citep{yao2026instructional}.
%%
%% =====================================================================
%% 【P2】Multimodal Interleaved 生成是当前热点（纯背景，不提gap）
%% =====================================================================
%%
%% 职责：只建立技术背景，说明这个方向很重要、很热，不在此段提任何gap。
%%
%% 逻辑：
%%   交织生成从两个维度同步推进——
%%
%%   生成侧：从早期MiniGPT-5 \citep{zheng2023interleaved}，
%%           到ANOLE \citep{chern2024anole}、Orthus \citep{kou2025orthus}，
%%           autoregressive交织生成能力持续演进
%%
%%   评测侧：OpenLEAF \citep{an2024openleaf}、MMIE \citep{xia2025mmie}
%%           建立了面向交织生成的统一评价标准
%%
%% → 段尾只说"这两个维度的发展表明交织生成正成为多模态AI的核心范式"，不提K-12
%%
%% =====================================================================
%% 【P3】交叉空白 = 交织生成 × K-12教育（两条线汇聚分析）
%% =====================================================================
%%
%% 职责：P3才是gap分析，从两个方向夹逼，说明交叉点是空白。
%%
%% 方向一（K-12教育内容生成）：
%%   现有K-12/教育生成工作要么纯文字: \citep{liu2025cogent}, \citep{wang2025genmentor}
%%   要么大学级别视频: \citep{ku2025theoremexplain}, \citep{chen2025code2video}
%%   → 没有K-12 STEM的交织图文生成
%%
%% 方向二（交织生成工作）：
%%   生成侧和评测侧（见P2）均聚焦通用场景（故事/how-to/网页），生成自然图像
%%   → 没有应用于K-12教育，且不要求几何精确的学科示意图
%%
%% 交叉空白：
%%   两条线发展迅猛，但交叉点无人涉及
%%   → Gap 1: 无系统将交织生成范式用于K-12 STEM内容生成
%%   → Gap 2: 无benchmark同时评K-12多模态内容的文字质量+视觉质量
%%            (\citep{kumar2025diagramir} 仅评数学图表，非K-12多学科；
%%             \citep{ji2025eduvis} 评可视化推理，非生成评测)
%%
%% =====================================================================
%% 【P4】我们的工作：EduIllustrate + EduIllustrateBench
%% =====================================================================
%%
%% EduIllustrate: 四阶段pipeline，multimodal interleaved输出（文字+图）
%% EduIllustrateBench: 覆盖3个年级，5个学科，配套8维评测rubric
%%
%% =====================================================================
%% 【P5】三条贡献
%% =====================================================================
%%
%% 1. 方法：首个K-12 STEM图文交织解析自动生成pipeline
%% 2. 评测：EduIllustrateBench（多学科、多年级benchmark + 8维rubric）
%% 3. 实验：5个前沿LLM对比，消融分析，human evaluation验证
%%
%% 
\end{comment}
...
【P1】
在K-12 STEM教育中，图文结合的解析材料对学生理解抽象概念至关重要。
认知负荷理论~\citep{sweller1988cognitive}、双重编码理论~\citep{paivio1971imagery}
与多媒体学习原则~\citep{mayer2001multimedia}均表明，将视觉图示与文字推理协同呈现，
能显著降低认知负担、提升概念理解深度。
然而，制作此类高质量图文解析材料面临重重障碍：
备课与材料制作是教师工作中最耗时的环节之一~\citep{philipp2013teachers,thompson2024teachers}，
而大量教师缺乏制作规范图示所需的专业技能~\citep{turkozudincer2025literacy}，
由此造成K-12学生可获取的优质图文解析材料的系统性短缺~\citep{yao2026instructional}。

【P2】
与此同时，多模态交织生成（interleaved image-text generation）正成为多模态AI研究的核心前沿，
并从两个维度同步推进：
在生成侧，从MiniGPT-5~\citep{zheng2023interleaved}
到ANOLE~\citep{chern2024anole}、Orthus~\citep{kou2025orthus}，
自回归交织生成能力持续演进；
在评测侧，OpenLEAF~\citep{an2024openleaf}与MMIE~\citep{xia2025mmie}
已建立面向交织生成的统一评价框架。
这两个维度的快速发展表明，交织生成正在成为多模态内容理解与生成的主流范式。

【P3】
然而，上述两条研究线在K-12 STEM教育场景下均存在系统性空白。
一方面，现有教育内容生成工作分化为两类：
文字生成系统（如COGENT~\citep{liu2025cogent}、GenMentor~\citep{wang2025genmentor}）
不包含视觉内容，
视频生成系统（如TheoremExplainAgent~\citep{ku2025theoremexplain}、
Code2Video~\citep{chen2025code2video}）则面向大学级别定理，
均未实现K-12 STEM的图文交织解析生成。
另一方面，现有交织生成工作聚焦于通用场景（故事、how-to、网页），
生成的视觉内容为自然图像，无法满足K-12学科示意图对几何精度和学科规范的要求。
两条线的交叉点至今空白。
此外，评测层面同样缺失：
现有图表评测工作（DiagramIR~\citep{kumar2025diagramir}）仅覆盖数学单科，
EduVisBench~\citep{ji2025eduvis}面向可视化推理而非生成质量评测，
尚无benchmark能够同时评估K-12多模态内容的文字质量与视觉质量。

【P4】
为填补上述空白，本文提出\textbf{EduIllustrate}——
首个面向K-12 STEM的多模态交织图文解析自动生成系统。
给定一道K-12 STEM题目，EduIllustrate通过四阶段pipeline输出
结构化的图文交织解析：文字推理步骤与学科示意图交替呈现，
示意图由程序化渲染生成，保证几何精度与跨图视觉一致性。
为支撑系统评测，我们构建\textbf{EduIllustrateBench}，
覆盖小学、初中、高中三个年级，数学、物理、化学、生物、地理五个学科，
并提出基于多媒体学习理论的\textbf{8维评测rubric}，
同时覆盖文字质量与视觉质量。

【P5】
本文的主要贡献包括：
（1）\textbf{方法}：提出首个K-12 STEM多模态交织图文解析自动生成pipeline，
引入顺序锚定策略保证跨图视觉一致性；
（2）\textbf{评测}：构建EduIllustrateBench及8维评测rubric，
填补K-12多学科多模态内容评测的空白；
（3）\textbf{实验}：对五个前沿LLM进行对比分析，
结合消融实验与人工评测验证系统有效性与LLM-as-judge可靠性。
...
We address these gaps with \textbf{EduIllustrate}, the first automated system for
...
\begin{itemize}
  \item \textbf{System}: EduIllustrate, the first pipeline for automated generation
  ...
  \item \textbf{Benchmark}: EduIllustrateBench with an 8-dimension evaluation rubric,
    addressing the gap in multi-subject, multi-grade multimodal educational content evaluation.
  \item \textbf{Experiments}: Comparative analysis of five frontier LLMs, ablation studies,
