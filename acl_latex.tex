...
\begin{itemize}
  ...
  \item  Comparative analysis of ten LLMs spanning proprietary and open-weight models, ablation studies,
...
Given a K-12 STEM problem statement (and an optional diagram) as input, the agent generates a structured Markdown document that interleaves programmatically rendered static diagrams with step-by-step, pedagogically scaffolded textual explanations.
...
\textbf{Stage 2: Scene Implementation Planning (Scene 1 only).} Applying detailed implementation planning to every scene would force each to independently invent its own visual conventions, making cross-scene consistency impossible to guarantee. We therefore restrict this stage exclusively to Scene 1, which serves as the visual anchor for the entire explanation. It converts the high-level \texttt{<SCENE\_1>} specification into a detailed implementation plan describing intended visual appearance in natural language, enumerating construction steps with explicit spatial constraints (safe area bounds, minimum element spacing, relative positioning), and specifying discipline-specific rendering conventions (right-angle markers in geometry, vector arrows in physics, VSEPR molecular geometry in chemistry). The resulting conventions---color scheme, labeling style, line weights, font sizes---are inherited by all subsequent scenes, ensuring a coherent visual language throughout the explanation.

\textbf{Stage 3: Code Generation and Rendering.} This stage translates implementation plans into executable Manim code and renders static PNG images. Scene 1 is generated first from its implementation plan; once complete, its full Manim code is passed as context to Scenes 2--$N$, which generate in parallel conditioned on this anchor. This design enforces visual consistency without requiring global optimization across all scenes simultaneously.
...
\textbf{Dataset Construction.} We construct a curated benchmark of 230 problems derived from K12-Vista~\citep{zhang2025k12vista}, spanning three grade levels (elementary, middle, high school) and five STEM subjects (Table~\ref{tab:dataset}). Problems are selected for diagram appropriateness, solution clarity, and knowledge point diversity; full curation details are provided in Appendix~\ref{sec:benchmark-details}.
